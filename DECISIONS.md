...
### AD-16 · Case 01's Evidence & attachments section is rendered from a single inline manifest
**Status:** Active  
**Date:** 2026-09-18  
**Decision:** The 23 artefact rows on `spe-case-01-nhs.html` are no longer hardcoded markup. They are rendered from one inline `const CASE_01_ARTEFACTS` array in the page, which is the single source of truth for the section. Each entry carries `id`, `phase`, `title`, `kind`, `description` and either `href` (`kind: 'document'`) or `body` (`kind: 'content'`); `kind: 'pending'` carries neither. Every row, every per-phase row count and every "N artefacts · …" label is derived from that array — adding an entry to it is the only step needed to add a row, and no count survives anywhere in the page or in the tests. The manifest is also exposed as `window.__CASE_01_ARTEFACTS__` so `qa_final.mjs` and `20-new-ui-crucible.spec.ts` derive their expectations from it rather than restating them.

The 21 slots that previously read `Not yet produced` are now filled. 19 of the 23 are `content`: no file was ever produced for them, so the substance itself — the old UI's artefact descriptions, findings, tables and figures — is rendered in the row as expandable content, in the case page's voice. Two remain `document` (the dashboard v5_1 prototype and the performance analytics microsite), now stating file type and size. Two remain `pending`, each with a stated reason rather than a bare empty state: `phase-02-six-stage-service-blueprint`, because no version of the blueprint exists in this repository to publish, and `phase-05-a-stated-model-quality-position`, because the accuracy conflict recorded in AD-17 is unresolved and no version of it should be published while that holds.

**Rationale:** Twenty-one rows reading `Not yet produced` was the page's weakest surface: the artefact section is the evidence that the framework produced something, and it was mostly empty. Hardcoded rows are also the reason it stayed empty — adding one meant editing markup in six places and keeping a count in prose in step with it, so nobody did. A manifest makes the section grow by adding data, and makes the row count a derived fact rather than a claim someone has to remember to update. Rendering the substance as `content` rather than waiting for files is the other half: for most of these artefacts no file was ever produced, so the content *is* the artefact, and a visitor can now read it.  
**Why an inline `const` rather than `public/new-ui/data/case-01-artefacts.json`:** a sibling JSON file would have to be read with `fetch`, and `qa_final.mjs` is deliberately portable across a dev server, a deployed preview *and* a `file://` path (its default base is `file://`). `fetch` of a sibling file from a `file://` document is blocked as a cross-origin request, so a JSON file would have made the section render only over HTTP and silently fail in one of its three supported run modes. The manifest is markup-adjacent data in a page that is otherwise self-contained static HTML, which is consistent with how these four pages already ship.

**Accessibility:** each `content` row is a native `<details>` / `<summary>` disclosure, so it is keyboard-operable and exposes its expanded state without scripting that behaviour. `aria-expanded` is mirrored on the summary on `toggle` (captured, since `toggle` does not bubble) because some screen readers still read it. Rows start collapsed. Wide tables sit in a focusable `overflow-x` region with an accessible name, so the page still measures 0px of horizontal overflow at 390px with every artefact expanded. Body sub-headings are `<h5>` under the block's `<h4>`, so the document outline holds with all 23 rows open.

**JavaScript off:** the rows are script-rendered, so each of the six phases carries a static `<noscript>` list naming its artefacts and their state, and linking the two attached prototypes directly. The section is never empty with scripting off. Acknowledged limitation: that fallback list is hand-maintained markup, so an entry added to the manifest later will appear in the rendered rows but not in the `<noscript>` list until someone updates it. This is the documented cost of the `<noscript>` option; the alternative — server-rendered rows the script hydrates — would have required a markup edit per new row, which is the thing this decision exists to remove. The fallback degrades by being incomplete, never by being wrong.

**Commercial exclusions:** no LOI value, named prospect trust, pricing, pipeline, revenue or addressable-market figure was migrated. Where an artefact was otherwise clean but carried one such figure, the clean material was migrated and the omission is stated in that row's own description and in a note at the foot of its body — the figure is omitted outright, never redacted or rounded. Both `qa_final.mjs` and the acceptance spec now assert the excluded terms are absent from the rendered page, so a future manifest edit cannot reintroduce one quietly. `ARR` and `LOI` are matched case-sensitively and on word boundaries, because a lower-cased substring search hits "narrative", "arrives" and "carried". `pipeline` is deliberately not matched as a bare word: this page already used it for the CI and ingest pipelines before this change, and the commercial exclusion does not cover that sense — the commercial senses (`sales pipeline`, `pipeline projects`) are matched instead.

**Consequence:**
- The `.evidence*` CSS is rewritten around the disclosure structure. The dead delegated upload/preview/remove JS handler that AD-15 left inert is removed, since the section no longer has any upload affordance for it to act on.
- `qa_final.mjs` goes from 66 to 85 checks over an HTTP base. All 19 new checks read their expectations from the manifest; none states a row count.
- `20-new-ui-crucible.spec.ts` gains an `AC-20b` describe block of 10 tests, likewise manifest-derived. Two `data-testid` hooks are added at the same time as the markup: `evidence-row` and `evidence-count`.
- Verified by adding a temporary manifest entry and confirming a new row appeared, expandable, with the phase count moving 3 → 4 and the label re-deriving, and no markup edited. That exercise also caught two over-strict assertions of my own — an id check that assumed the manifest array was pre-grouped by phase, and a 200-character floor on body length that would have failed a legitimately short artefact. Both were corrected rather than worked around, then the probe was removed.
- `spe-framework.html`, `spe-case-foundry.html` and `spe-resources.html` are untouched, so AD-13's byte-identical claim for the foundry and resources pages still holds and AD-14's correction still describes the framework page accurately.

**Where:** `public/new-ui/spe-case-01-nhs.html`, `qa/qa_final.mjs`, `tests/acceptance/20-new-ui-crucible.spec.ts`.

---

### AD-17 · The classification-accuracy conflict is recorded, not resolved
**Status:** Open  
**Date:** 2026-09-18  
**Decision:** The old UI and the case page state different classification-accuracy figures, and neither is migrated onto the case page while that is true. `src/app/data/seedData.ts` records 89% observed overall with a 91% target in `artefact-findings`, a ≥91% overall / ≥88% per-theme requirement in `artefact-prd`, a 91% accuracy key result in `artefact-okrs`, and — in the same findings artefact — a "92% PRD target" that its own PRD artefact puts at 91%. `spe-case-01-nhs.html` states 83.4% observed against a 91% target, and presents the withdrawal of the 91% claim as one of the three things the framework killed. The per-theme spike breakdown carries a further set of figures again.

No attempt is made here to reconcile them. `phase-05-a-stated-model-quality-position` is therefore left as `kind: 'pending'` with that reason stated in the row, finding F2's accuracy claims are excluded while its non-accuracy content (the taxonomy split, and the two-day engineering estimate) is migrated, and the accuracy rows are dropped from the migrated PRD non-functional requirements and OKR key results with the omission noted in each row.  
**Rationale:** Publishing either figure would assert a position nobody has taken, and publishing both would put a contradiction on a client-facing page. The page already says, in "What this case does not claim", that no labelled validation dataset exists — which is the honest state. Recording the conflict is what makes it findable when someone does resolve it; the alternative is a reviewer discovering it and not knowing whether it is a defect or a decision.  
**Where:** `src/app/data/seedData.ts` (lines ~586–605, ~669–716, ~363–396), `public/new-ui/spe-case-01-nhs.html`.

---

### AD-18 · Strategy pack stays unpublished while production is unprotected
**Status:** Open  
**Date:** 2026-09-18  
**Decision:** `src/imports/Patient_Feedback_Platform_PM_Strategy_Pack.pdf` (20 pages) is the only remaining source of real file attachments for this case, and it is still not published. Production protection was re-checked on 2026-09-18: `https://refine-problem-statement.vercel.app/new-ui/spe-framework.html` returns HTTP 200 with full page content and no redirect to SSO, so `/new-ui/` remains publicly readable and the condition AD-15 recorded is unchanged. The page-to-slot mapping and the list of pages carrying commercial figures are prepared and recorded in the pull request, but nothing has been extracted, split, converted or copied into `public/`.  
**Rationale:** Pages 2, 4, 6, 16 and 18 of the pack carry market sizing, revenue objectives and key results, the pricing research method, and the commercial gate verdict including the LOI value and the named prospect trust. Publishing the pack whole is a business decision, not a build one. Publishing it in part means producing a derived artefact, which is worth doing once — after protection is enabled, so the derived file does not have to be treated as public from the moment it lands.  
**Where:** `src/imports/Patient_Feedback_Platform_PM_Strategy_Pack.pdf` (not served).
